...
async function createHttpServer(listenPort: number) {
  ...
  app.get('/api/v3/delta', (req, res) => {
    ...
    try {
      ...
      fs.writeFileSync(
        `${workdir}/auth.json`,
        { auths: { [registry]: { auth } } }.toString()
      );
      fs.copyFileSync(`/usr/local/deltaimage`, `${workdir}/deltaimage`);
      const diff = execSync(
        `deltaimage docker-file diff ${src} ${dest}`
      ).toString();
      fs.writeFileSync(
        `${workdir}/Dockerfie.diff`,
        diff.replace(/--from=deltaimage\/deltaimage:0.1.0 \/opt/g, '.'),
        'utf8'
      );
      execSync(
        `buildah bud --authfile ${authfile} --no-cache -f ${workdir}/Dockerfie.diff -t ${uuid}-diff ${workdir}`
      );
      const delta = execSync(
        `deltaimage docker-file apply ${uuid}-diff`
      ).toString();
      fs.writeFileSync(
        `${workdir}/Dockerfie.delta`,
        delta.replace(/--from=deltaimage\/deltaimage:0.1.0 \/opt/g, '.'),
        'utf8'
      );
      execSync(
        `buildah bud --authfile ${authfile} --no-cache -f ${workdir}/Dockerfie.delta -t ${dest}:delta-XYZ ${workdir}`
      );
